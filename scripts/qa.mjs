...
for(const L of lessons){
  ...
  const exq=(L.examples||[]).map(x=>String(x.question||'').trim());
  if(new Set(exq).size!==exq.length) fail.push(label+': duplicate worked example');
  const qs=[...(L.practice||[]),...(L.homework||[])].map(x=>String(x).trim());
  const seen=new Map();
  qs.forEach((q,i)=>{if(seen.has(q))fail.push(label+': duplicate question "'+q+'"');else seen.set(q,i)});
...
for(const id of [52,53,54,55,56]){
  if(!lessons.find(x=>x.lesson===id)) fail.push('Missing statistics lesson '+id);
}
if(fail.length){
  console.error(fail.join('\n'));
  process.exit(1);
}
console.log('PASS: 70 lessons; progressive v4 overrides applied; 24 practice + 14 homework questions each; examples and answer banks aligned.');
